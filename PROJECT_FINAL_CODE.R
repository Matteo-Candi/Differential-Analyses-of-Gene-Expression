...
# Print the histogram of the degree together with a line for the 98% quantile
quantile_hubs <- .99
x_n <- quantile(d.n, quantile_hubs)

data.n <- data.frame(value = unlist(d.n))
row.names(head(sort(data.n, decreasing = T), 10))
...
row.names(head(sort(data.c, decreasing = T), 10))
...
for (i in 1:length(hubs.n)){hubs.n.ids[i] <- match(names(hubs.n)[i],rownames(adj.mat.n))}
...
# Create the sub-network composed by the Hubs and their neighbors for Normal cancer
...
write.csv(hub.n.adj, file = "networks/hub_diff_adj.csv", row.names = FALSE)
...
# new_update
# adj.mat.cp <- rho.cp * (qval.cp <= 0.025)
...
selected.patiets <- colnames(adj.mat.cp)
cacer.stage.info <- subset(cancer.stage.info, submitter_id %in% selected.patiets)
