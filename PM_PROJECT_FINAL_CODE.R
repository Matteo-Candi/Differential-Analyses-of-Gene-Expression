...
# 1 - Load the data just as seen in the lab ----

# Project id to use to load the data
proj <- "TCGA-CHOL"

# Create a folder to store the data
dir.create(file.path(proj))

# Look for all data linked to a "Primary Tumor" sample
...
dim(rna.expr.data.C)
length(unique(substr(colnames(rna.expr.data.C), 1,12))) #there are duplicates for cancer tissue instead

# There are some duplicate data in the "primal tumor" class
...
sort(table(patients.C)) # There are 3 patients that appear 3 times
...
# and keep only the unique patients
...
length(intersect(colnames(expr.N), colnames(expr.C))) # 18
# Out of the 19 patients for which we have the normal sample for 18 of them we
# also have the cancer one


# Since we want to do a comparison between the sane and ill patients we need
# to get rid of the one for which we don't have the "primal tumor" sample
...
# we want integers as values of the lists
typeof(expr.C[1,1]) 
typeof(expr.N[1,1]) 

# And we don't want any missing value
any(is.na(expr.C)) 
...
sum(rowSums(normalized_counts == 0) == 36) #no null rows
...
filtr.expr.n <- as.data.frame(normalized_counts[, 1:18])
filtr.expr.c <- as.data.frame(normalized_counts[, 19:36])
...
length(deg.genes) # We have 580 different genes
...
expr.table$diffexpressed[expr.table$fc >= threshold.fc & expr.table$pval.fc.fdr <= 0.01] <- "UP"
# the genes with FC <= -threshold.fc
expr.table$diffexpressed[expr.table$fc <= -threshold.fc & expr.table$pval.fc.fdr <= 0.01] <- "DOWN"
...
ggplot(data=expr.table, aes(x=fc, y=-log10(pval.fc.fdr), col=diffexpressed))+  
  ...
  geom_vline(xintercept=threshold.fc, col="red")+
  geom_vline(xintercept=-threshold.fc, col="red")


# 3 - Co-expression networks -----

## 3 - Computation of the Adjacency matrices ----
...
# Correlation matrix for the cancer network - we will use Pearson correlation now
...
network.size(net.n) # 580
network.edgecount(net.n) #14265

# and the density of the network
network.density(net.n) # 0.08495623

# Same for the CANCER NETWORK
net.c <- as.network(adj.mat.c, directed = FALSE)

network.size(net.c) # 580
network.edgecount(net.c) # 1696

network.density(net.c) # 0.01010065

# The normal network is much more dense than the cancer one. 
...
hist(d.c,col = "lightblue", main = "Degree distribution - cancer tissue")
abline(v = x_c, col="red")
# It could be scale free
...
## 3 - Compare the hubs sets -----

common <- intersect(names(hubs.c),names(hubs.n))
length(common) # 3 hubs in common
...
# Apply Fisher z-transform to the correlation matrices

z.c <- 0.5*log((1 + rho.c)/(1- rho.c))
z.n <- 0.5*log((1 + rho.n)/(1- rho.n))

# Check that the output dimension is the expected one 
...
network.size(net.diff) #580 (as expected)
network.edgecount(net.diff) #5864

network.density(net.diff) #0.03492347
...
x_d <- quantile(d.diff[d.diff>0],0.95)
hist(d.diff,col = "lightblue", main = "Degree distribution - differential co-expressed network", xlab = 'Degree')
abline(v=x_d, col="red")
text(x = x_d, y = par("usr")[4]*0.95, labels = "0.95 Quantile", pos = 2, offset = 1, col = "red", cex = 1)
...
intersect(names(hubs.diff), not_in_common_c) #0
intersect(names(hubs.diff), not_in_common_n) #5
...
text(x = 1.5, y = -1, labels = "slope = -0.67", pos = 2, offset = 1, col = "red", cex = 1)
...
network.density(net.hub.diff) # 0.146707
...
# 5 PSN and Community detection + PSN of normal tissue ---------------------
...
#adj.mat.cp <- rho.cp * (qval.cp <= 1e-20)  

adj.mat.cp <- rho.cp * (abs(rho.cp) > 0.7)   #since the pvalues are all extremely small we can try to
                                              # define the adjacency only looking at the correlation value
...
# 1 - compute a different centrality index and check overlap with previous hubs -----
...
# 2 - Enrichment analysis on cancer nodes ---------------  

# Extract common names of the genes 

hubs.c_names <- genes.info[names(hubs.c), ]$gene_name

#not_in_common_c_names <- genes.info[not_in_common_c, ]$gene_name
...
# 3 - repeat the analysis with spearman correlation --------------------------------------


# Co-expression networks -----

## Computation of the Adj matrices ----
