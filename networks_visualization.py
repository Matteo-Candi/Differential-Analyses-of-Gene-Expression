...
def plot_communities():
    ...
    df = pd.read_csv(f'networks/community_adj.csv')
    cancer_stage = pd.read_csv(f'networks/cancer_stage.csv')
    nodes = list(df.columns)
    colors = ['#2EBFA5', '#F26419', '#FDCA40', 'darkgray']

    with open('output.txt', 'r') as file:
        communities = []
        for line in file:
            communities.append(int(line)-1)

    G = nx.Graph()

    for i, node in enumerate(nodes):
        stage = cancer_stage[cancer_stage['name']==node]['stage'].values[0][6:]
        G.add_node(node, name=node, color = colors[communities[i]], community=communities[i], stage = stage)

    for col, row in df.iterrows():
        for index, value in row.items():
            if value:  
                G.add_edge(nodes[col], index, weight=value)

    
    node_colors = [G.nodes[node]['color'] for node in nodes]
    edge_weights = [data['weight'] for _, _, data in G.edges(data=True)]
    labels = nx.get_node_attributes(G, 'stage')

    pos = nx.spring_layout(G)


    nx.draw(G, pos, with_labels=True, labels=labels, node_color=node_colors, node_size=200, edge_color='lightgrey', width=edge_weights, arrows=False)
    plt.show()
